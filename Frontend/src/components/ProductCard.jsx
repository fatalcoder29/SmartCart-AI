      ...
      <article className="relative">
        <div className="product-shine relative aspect-[3/4] overflow-hidden bg-cream-dark rounded-xl">
          ...
          <button
            ...
            className="absolute top-3 right-3 z-10 rounded-full bg-cream/90 p-2 text-ink shadow-sm backdrop-blur-sm transition-all duration-200 hover:scale-110 hover:bg-cream"
          >
            <Heart
          ...
          {showAddButton && (
            <button
              type="button"
              onClick={handleAdd}
              className="absolute inset-x-2 bottom-2 flex items-center justify-center gap-1.5 rounded-lg bg-cream py-2.5 text-[11px] font-medium tracking-wide shadow-md transition-all duration-300 sm:inset-x-3 sm:bottom-3 sm:translate-y-3 sm:py-3 sm:text-[12px] sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100 hover:bg-ink hover:text-cream"
            >
        ...
        <div className="mt-3 flex items-start justify-between gap-2 sm:mt-4 sm:items-baseline sm:gap-3">
          <h3 className="font-display text-sm font-medium leading-snug transition-colors group-hover:text-terracotta sm:text-base md:text-lg">
            {product.name}
          </h3>
          <div className="flex shrink-0 flex-col items-end gap-0.5 sm:flex-row sm:items-baseline sm:gap-1.5">
            {hasDiscount && product.originalPrice && (
              <span className="text-[10px] text-ink-muted line-through sm:text-xs">
                {formatPrice(product.originalPrice)}
              </span>
            )}
            <p className={`text-xs font-medium sm:text-sm ${hasDiscount ? 'text-terracotta' : 'text-ink-muted'}`}>
              {formatPrice(product.price)}
            </p>
          </div>
        </div>
        <p className="mt-0.5 text-[10px] capitalize tracking-wide text-ink-muted/80 sm:mt-1 sm:text-xs">
          {product.category}
        </p>
      </article>
    </Link>
